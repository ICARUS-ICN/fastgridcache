...
trait Searcher {
    ...
    fn try_advance_index(&mut self, index: usize) -> bool {
        ...
        let cost = cost_full(self.filtered().get_raw_ref().iter().copied());
        self.filtered_mut()[index] += 1;
        let new_cost = cost_full(self.filtered().get_raw_ref().iter().copied());
    ...
    fn get_cost(&self) -> u64 {
        ...
        cache_costs + cost_full(self.filtered().get_raw_ref().iter().copied())
    }
...
#[cfg(test)]
mod tests {
    use crate::gridsearcher::get_cache_locations;
